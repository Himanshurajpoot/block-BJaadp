let persons = [
  ...
  let totalGrade=persons.reduce((acc,cv)=>{return acc+cv.grade},0)
  console.log(totalGrade/personsLn)
  // Find the average grade of male
let maleGradeAvg = persons
.filter((element)=>element.sex==='M')
.reduce((acc,cv)=>acc+cv.grade,0)/personsLn
console.log(maleGradeAvg)
  // Find the average grade of female
  let femaleGradeAvg = persons
.filter((element)=>element.sex==='F')
.reduce((acc,cv)=>acc+cv.grade,0)/personsLn
console.log(femaleGradeAvg)
  // Find the highest grade
 let highestgrade = persons.map((a)=>a.grade).sort((a,b)=>a-b).pop()
 console.log(highestgrade)
  // Find the highest grade in male
  let highestgradeMale=persons.filter((element)=>element.sex==='M')
  .map((a)=>a.grade)
  .sort((a,b)=>a-b).pop()
  ...
  let findhigh = persons
  .filter((element)=>element.name.startsWith('J')||element.name.startsWith('P'))
  .map((a)=>a.grade)
  .sort((a,b)=>a-b)
  .pop();
...
console.log(fruitsObj)
  ...
  [['banana', 2], ['cherry', 3], ['orange', 3], ['apple', 2], ['fig', 1]]
  */
console.log(Object.keys(fruitsObj))
...
function increment(num){
 ...
 function half(num){
  ...
  EXAMPLE:
    ...
    ...
  */
// pipeline.forEach(elm=>console.log(elm(34)))
let cv= pipeline.reduce((acc,cv)=>cv(acc),3)
console.log(cv)
  ...
  let cv1= pipeline2.reduce((acc,cv)=>cv(acc),8)
console.log(cv1)